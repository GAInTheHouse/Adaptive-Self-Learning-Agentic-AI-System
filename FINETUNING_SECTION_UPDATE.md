# Fine-Tuning Section Update for GenAI Final Report

## Position in Report
**Replace the existing "LoRA-Based Parameter-Efficient Fine-Tuning" section** with the following content.

---

## Updated Section Text (Copy-Paste Ready for Overleaf)
...
1. **Citation**: You'll need to add a citation entry for the Edinburgh DataShare dataset in your bibliography. Suggested format:
   ```bibtex
   @dataset{edinburgh_noisy_speech,
...
2. **LaTeX Packages**: Ensure you have the necessary packages:
   - `\usepackage{url}` for URLs
   - `\usepackage{hyperref}` if you want clickable links

3. **Section Placement**: This should replace your existing "LoRA-Based Parameter-Efficient Fine-Tuning" section. The exact section number will depend on your document structure.

4. **Customization**: You may want to adjust:
   - Mathematical notation style (currently uses inline math for numbers)
   - Citation style to match your document
   - Technical detail level (currently balanced for a report)
